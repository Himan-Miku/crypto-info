...
const Home = () => {
  const { data, isLoading, error, isError } = useQuery(["coins"], async () => {
    const response = await Axios.get(
      "https://coinranking1.p.rapidapi.com/coins?referenceCurrencyUuid=yhjMzLPhuIDl&timePeriod=24h&tiers%5B0%5D=1&orderBy=marketCap&orderDirection=desc&limit=12&offset=0",
      config
    );
    ...
    <div className="w-[79vw] bg-gray-500 overflow-y-scroll h-screen scrollbar-hide pb-8">
      <div className="flex flex-col gap-6 mt-8">
        <h2 className="text-center text-white text-2xl font-medium bg-gray-800 w-[50%] mx-auto p-[0.88rem] rounded-xl">
          Top 12 Crypto Coins in the World
        </h2>
